...
const Users = () => {
            ...
            <div className="flex flex-row justify-between items-center gap-3">
                <div className="flex align-items-center gap-3 flex-wrap">
                    <div className="flex align-items-center gap-2">
                        ...
                        <select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)} className="p-2 border-1 surface-border border-round" style={{ minWidth: '150px', padding: '0.5rem 0.75rem' }}>
    ...
    const isAcceptedBodyTemplate = (rowData: UserType) => {
        return <span>{rowData.isAccepted ? 'Active' : 'Pending'}</span>;
    };
    ...
    const handleBulkToggle = async (status: 'Active' | 'Pending') => {
        ...
        setUsers((prev) => prev.map((u) => (uids.includes(u.uid) ? { ...u, isAccepted: newValue } : u)));
        setSelectedUsers([]);
    };
    ...
    const displayUsers = users
        .filter((user) => globalFilterValue === '' || user.displayName?.toLowerCase().includes(globalFilterValue.toLowerCase()) || user.email?.toLowerCase().includes(globalFilterValue.toLowerCase()))
                        ...
                        <Column
                            ...
                            body={(rowData) => (
                                <span
                                    ...
                                    className="border-b border-white text-blue-400 pointer font-bold"
                                >
                                    {rowData.displayName}
                                </span>
                            )}
